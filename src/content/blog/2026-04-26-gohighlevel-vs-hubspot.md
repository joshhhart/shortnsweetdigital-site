...
1. **Week 1**: Export contacts, custom properties, deal stages, and email templates from HubSpot. Map them to GHL's pipeline + custom field structure.
2. **Week 2**: Rebuild the top 5 most-used automations in GHL's workflow builder. Reconnect SMS, web chat, and calendar booking. Migrate 1 staff member to use GHL exclusively.
3. **Week 3**: Cut the rest of the team over. Run HubSpot in read-only mode for historical lookup. Verify that no inbound channels are still pointing at HubSpot endpoints.
4. **Week 4**: Cancel HubSpot at the end of the billing period (do not auto-renew). Archive the export to a private S3 bucket for compliance.
